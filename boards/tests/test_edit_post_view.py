...
class PostUpdateViewTestCase(TestCase):
	...
	def setUp(self):
		...
		self.post = Post.objects.create(message='Some test message', topic=self.topic, created_by=user)
...
class UnauthorizedPostUpdateViewTests(PostUpdateViewTestCase):
	...
	def setUp(self):
		super().setUp()
		username = 'jennifer'
		password = 'jennifers_password'
		user = User.objects.create_user(username=username, email='jennifer@dou.com', password=password)
...
class SuccessfulPostUpdateViewTests(PostUpdateViewTestCase):
	...
	def setUp(self):
		super().setUp()
		self.client.login(username=self.username, password=self.password)
		self.response = self.client.post(self.url+'?next=/boards/1/topics/1/', {'message': 'edited message'})
